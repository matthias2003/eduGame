...
MyCharacter::MyCharacter(std::string& name, double power, double health, double defense, double luck, double gold, double xp, double level, double statsAdded) {
    this->name = name;
    this->power = power;
    this->health = health;
...
void MyCharacter::AddStats() {
    ...
    if (this->gold >= cost) {
        while (again) {
            ...
            std::vector<std::string> options = { "Power","Health","Defense","Luck" };
            for (int i = 0; i < 4; i++) {
